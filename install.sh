...
check_root() {
    # Check for su binary
    if [ -e "/system/xbin/su" ] || [ -e "/system/bin/su" ]; then
        echo "Root access detected (su binary found)"
        return 0
    fi

    # Check for busybox
    if [ -x "$(command -v busybox)" ]; then
        echo "Root access detected (busybox found)"
        return 0
    fi

    # Check for writable system partition
    if [ -w "/system" ] || [ -w "/vendor" ]; then
        echo "Root access detected (writable system partition)"
        return 0
    fi

    # Check for SELinux permissive mode
    if [ "$(getenforce)" == "Permissive" ]; then
        echo "Root access detected (SELinux permissive mode)"
        return 0
    fi

    # If no root is detected, exit the script
...
echo ""
printf "${C}╔══════════════════════════════════════╗${W}\n"
printf "${C}║  UAPE v3.0  Self-Contained Installer ║${W}\n"
printf "${C}╚══════════════════════════════════════╝${W}\n"
echo ""

[ "$(id -u)" -ne 0 ] && die "Root required. Run via Magisk/KernelSU terminal."
ok "Root confirmed"

RF="Unknown"
[ -d /data/adb/magisk ] && RF="Magisk"
[ -d /data/adb/ksu    ] && RF="KernelSU"
[ -d /data/adb/ap     ] && RF="APatch"
say "Framework: $RF"

if [ -f "$PID_FILE" ] && [ -d "/proc/$(cat "$PID_FILE" 2>/dev/null)" ]; then
    warn "Stopping existing UAPE (PID=$(cat "$PID_FILE"))..."
    kill "$(cat "$PID_FILE")" 2>/dev/null
    sleep 2
fi

say "Creating directories..."
mkdir -p "$BASE_DIR" "$PROF_DIR" "$AI_DIR" || die "Cannot create directories"
ok "Directories ready"

say "Writing service.sh..."
cat > "$SERVICE_DST" << 'SERVICE_EOF'
#!/system/bin/sh
...
chmod 755 "$SERVICE_DST" || die "chmod failed"
ok "service.sh installed: $SERVICE_DST"

if [ ! -f "$CONF" ]; then
    say "Writing default config..."
    cat > "$CONF" << 'CONF_EOF'
# ══════════════════════════════════════════════════
# UAPE v3 — optimizer.conf
# Engine NEVER overclocks. Only min_freq (floor) moves.
# Restart engine after editing.
# ══════════════════════════════════════════════════

# FPS target (0 = auto-detect, or set manually e.g. 60 or 90)
TARGET_FPS=0
...
CONF_EOF
    ...
    warn "Config exists — keeping your settings"
fi

say "Registering auto-start..."
_sd_dir="$BASE_DIR/service.d"
_launcher="$_sd_dir/99-uape.sh"
_registered=0

if mkdir -p "$_sd_dir" 2>/dev/null; then
    cat > "$_launcher" << 'LAUNCH_EOF'
#!/system/bin/sh
(sh /data/adb/service.sh >> /data/adb/optimizer.log 2>&1) &
LAUNCH_EOF
    chmod 755 "$_launcher"
    ok "Auto-start: $_launcher"
    _registered=1
fi

if [ "$_registered" -eq 0 ] && [ -d /system/etc/init.d ] && [ -w /system/etc/init.d ]; then
    cat > /system/etc/init.d/99uape << 'LAUNCH_EOF'
#!/system/bin/sh
(sh /data/adb/service.sh >> /data/adb/optimizer.log 2>&1) &
LAUNCH_EOF
    chmod 755 /system/etc/init.d/99uape
    ok "Auto-start: /system/etc/init.d/99uape"
    _registered=1
fi

[ "$_registered" -eq 0 ] && warn "No auto-start slot found. Start manually: sh $SERVICE_DST &"

say "Starting UAPE engine..."
touch "$LOG"
(sh "$SERVICE_DST" >> "$LOG" 2>&1) &
sleep 3

if [ -f "$PID_FILE" ] && [ -d "/proc/$(cat "$PID_FILE" 2>/dev/null)" ]; then
    ok "Engine running  PID=$(cat "$PID_FILE")"
else
    warn "Engine still initializing — check: tail -f $LOG"
fi

echo ""
printf "${G}╔══════════════════════════════════════╗${W}\n"
printf "${G}║  Installation complete!              ║${W}\n"
printf "${G}╚══════════════════════════════════════╝${W}\n"
echo ""
echo "  Config   →  $CONF"
echo "  Log      →  $LOG"
...
echo "    Dump stats  → kill -USR1 \$(cat $PID_FILE)"
echo "    Stop engine → kill \$(cat $PID_FILE)"
echo ""
